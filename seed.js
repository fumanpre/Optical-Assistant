...
const documents = [
  "Optical-Assist is a Canadian practice management software designed for optometry clinics.",
  "Patients should avoid driving for 4-6 hours after pupil dilation.",
  "Our clinic accepts SunLife, BlueCross, and Manulife insurance providers.",
  "Patient data is encrypted both in transit and at rest.",
];
...
async function seed() {
  ...
  for (let doc of documents) {
    const fileHash = crypto.createHash("sha256").update(doc).digest("hex");

    const docInsert = await client.query(
      `INSERT INTO documents (filename, file_hash, file_size)
       VALUES ($1, $2, $3)
       RETURNING id`,
      ["seeded-doc", fileHash, doc.length]
    );
